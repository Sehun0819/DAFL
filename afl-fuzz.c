...
#include "gpf/afl.h"
#include "gpf/engine.h"
#include "gpf/target_language.h"
...
/* Read all testcases from buffer, then queue them for testing.
   Called at switching to AFL. */

// static void read_testcases_from_buffer(const std::vector<std::string>& tc_filepaths) {

//   u32 i;
//   u8* fn;
//   u32 n_new_tc = 0;

//   for (i = 0; i < tc_filepaths.size(); i++) {

//     struct stat st;

//     u8* fn = alloc_printf("%s", tc_filepaths[i].c_str());
//     if (!access(fn, F_OK)) FATAL("Invalid seed '%s'", fn);
...
//     SAYF("\n" cLRD "[-] " cRST
//          "Looks like there are no new test cases generated by GPF.\n");
...
#define SP20    SP10 SP10
  ...
  /* show PathFinder stats */
  if (gpf_handler::terminal[0] != '\0') {
    SAYF(cYEL "GPF\n" cRST);
    SAYF("%s\n", gpf_handler::terminal);
  }
...
EXP_ST void check_binary(u8* fname) {
  ...
  // if (memmem(f_data, f_len, GPF_DEFER_SIG, strlen(GPF_DEFER_SIG) + 1)) {

  //   OKF(cPIN "Deferred GPF trace on binary detected.");
  //   setenv(GPF_DEFER_ENV_VAR, "1", 1);
...
int main(int argc, char** argv) {
  ...
  // print_nd_branches(target_path, pcid_to_pc, nd_pcids);

  // TODO: mutate seeds and classify <original, mutant> pairs by their branch points

  // pf_engine.read_seeds(std::string((char*)in_dir));
  // pf_engine.reset_counter();
  // pf_engine.warmingup();
  ...
  while (1) {
    ...
    rsrc_schedule(afl_iter, afl_tmout, pf_iter, pf_tmout);
